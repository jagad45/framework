...
from modules.hash_cracker.core import Core
...
class Apps(BaseModules):
    ...
    def initialize(self):
        ...
        option_pathlist.set_shortdesc("Set wordlist")
        option_pathlist.set_values(File("/home/billal/Projects/JagadFramework/JagadFramework/system/wordlist/abstract.txt"))
        ...
        option_type = OptionsDict("type", "sha1", "Set hash type")
    ...
    def onRun(self, argument: dict = {}):
        self.core.reset()
        self.core.paste_options(self.options.parse())
        isSupport = self.core.check_algo()
        if isSupport is True:
            isValid = self.core.validate_hash()
            if isValid:
                isRead = self.core.check_wordlist()
                if isRead:
                    print()
                    status = self.core.run()
                    return status
                else: return False
            else:
                return False
    ...
    #     if argument_length == 0:
    #         self.core.paste_options(self.options.parse())
    #         self.core.read_wordlist()
    #         new_wordlist = self.core.options["wordlist"]
    #         option = self.options.exists("wordlist")
    #         options = self.options.get_opts()
    #         index_value = options.index(option)
            
    #         new_option = OptionsDict(options[index_value].option, new_wordlist, options[index_value].shortdesc)
    #         self.options.delete(index_value)
    #         self.options.set_opts(new_option)
    #         self.core.reset()
    #         return True
